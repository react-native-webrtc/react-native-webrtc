...
import com.oney.rnwebrtc.factories.H264AndSoftwareVideoDecoderFactory;
import com.oney.rnwebrtc.factories.H264AndSoftwareVideoEncoderFactory;

import com.oney.rnwebrtc.utils.EglUtils;
import com.oney.rnwebrtc.utils.ReactBridgeUtil;
import com.oney.rnwebrtc.utils.SerializeUtils;
import com.oney.rnwebrtc.utils.StringUtils;
...
public class WebRTCModuleImpl {
    ...
    public static final String NAME = "WebRTC";
    ...
    private final SparseArray<PeerConnectionObserver> peerConnectionObservers;
    final Map<String, MediaStream> localStreams;

    private final GetUserMediaImpl getUserMediaImpl;

    public WebRTCModuleImpl(ReactApplicationContext context) {
        reactContext = context;
        peerConnectionObservers = new SparseArray<>();
        ...
        if (adm == null) {
            adm = JavaAudioDeviceModule.builder(context).setEnableVolumeLogger(false).createAudioDeviceModule();
        }
    ...
    private PeerConnection getPeerConnection(int id) {
        PeerConnectionObserver pco = peerConnectionObservers.get(id);
    ...
    MediaStream getStreamForReactTag(String streamReactTag) {
        ...
        try {
            return (MediaStream) ThreadUtils.submitToExecutor((Callable<Object>) () -> {
                ...
                for (int i = 0, size = peerConnectionObservers.size(); i < size; i++) {
                    PeerConnectionObserver pco = peerConnectionObservers.valueAt(i);
    ...
    public MediaStreamTrack getTrack(int peerConnectionId, String trackId) {
        if (peerConnectionId == -1) {
            return getLocalTrack(trackId);
        }

        PeerConnectionObserver pco = peerConnectionObservers.get(peerConnectionId);
        if (pco == null) {
    ...
    private ReadableArray getTransceiversInfo(PeerConnection peerConnection) {
        ...
        for (RtpTransceiver transceiver : peerConnection.getTransceivers()) {
            ...
            transceiverUpdate.putMap("senderRtpParameters", SerializeUtils.serializeRtpParameters(transceiver.getSender().getParameters()));
            transceiverUpdate.putMap("receiverRtpParameters", SerializeUtils.serializeRtpParameters(transceiver.getReceiver().getParameters()));
    ...
    public boolean peerConnectionInit(ReadableMap configuration, int id) {
        ...
        try {
            return (boolean) ThreadUtils.submitToExecutor(() -> {
                PeerConnectionObserver pco = new PeerConnectionObserver(this, id);
                PeerConnection peerConnection = peerConnectionFactory.createPeerConnection(rtcConfiguration, pco);
                if (peerConnection == null) {
                    return false;
                }

                pco.setPeerConnection(peerConnection);
                peerConnectionObservers.put(id, pco);
                return true;
            }).get();
        } catch (ExecutionException | InterruptedException e) {
            e.printStackTrace();
        ...
        ThreadUtils.runOnExecutor(() -> {
            ...
            try {
                ...
                if (direction.equals(RtpTransceiver.RtpTransceiverDirection.SEND_RECV)
                    ...
                    RtpCapabilities capabilities = peerConnectionFactory.getRtpSenderCapabilities(transceiver.getMediaType());
                ...
                if (direction.equals(RtpTransceiver.RtpTransceiverDirection.SEND_RECV)
                    ...
                    RtpCapabilities capabilities = peerConnectionFactory.getRtpReceiverCapabilities(transceiver.getMediaType());
                    for (RtpCapabilities.CodecCapability codec : capabilities.codecs) {
                        Map<String, Object> codecDict = SerializeUtils.serializeRtpCapabilitiesCodec(codec).toHashMap();
                        availableCodecs.add(new Pair<>(codecDict, codec));
                    }
    ...
    public void enumerateDevices(Promise promise) {
        ThreadUtils.runOnExecutor(() -> getUserMediaImpl.enumerateDevices(promise));
    }

    public void getUserMedia(ReadableMap constraints, Promise promise) {
        ThreadUtils.runOnExecutor(() -> getUserMediaImpl.getUserMedia(constraints, promise));
    }
    ...
    public void mediaStreamCreate(String id) {
        ThreadUtils.runOnExecutor(() -> {
            MediaStream mediaStream = peerConnectionFactory.createLocalMediaStream(id);
    ...
    public void mediaStreamAddTrack(String streamId, int peerConnectionId, String trackId) {
        ThreadUtils.runOnExecutor(() -> {
            ...
            MediaStreamTrack track = getTrack(peerConnectionId, trackId);
            if (track == null) {
    ...
    public void mediaStreamTrackSetEnabled(int peerConnectionId, String id, boolean enabled) {
        ThreadUtils.runOnExecutor(() -> {
            MediaStreamTrack track = getTrack(peerConnectionId, id);
            if (track == null) {
    ...
    public WritableMap receiverGetCapabilities(String kind) {
        try {
            return (WritableMap) ThreadUtils.submitToExecutor((Callable<Object>) () -> {
                ...
                RtpCapabilities capabilities = peerConnectionFactory.getRtpReceiverCapabilities(mediaType);
    ...
    public WritableMap senderGetCapabilities(String kind) {
        try {
            return (WritableMap) ThreadUtils.submitToExecutor((Callable<Object>) () -> {
                ...
                RtpCapabilities capabilities = peerConnectionFactory.getRtpSenderCapabilities(mediaType);
                return SerializeUtils.serializeRtpCapabilities(capabilities);
            }).get();
        } catch (ExecutionException | InterruptedException e) {
    ...
    public void peerConnectionDispose(int id) {
        ThreadUtils.runOnExecutor(() -> {
            ...
            pco.dispose();
            peerConnectionObservers.remove(id);
        });
    }
