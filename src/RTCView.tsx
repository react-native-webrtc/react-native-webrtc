...
import NativeRTCVideoView from './NativeRTCVideoView';
...
type NativeProps = React.ComponentProps<typeof NativeRTCVideoView>;

const RTCView = forwardRef<any, RTCVideoViewProps>((props, ref) => {
    const { iosPIP, onDimensionsChange, zOrder, objectFit, ...rest } = props;

    const nativeProps: NativeProps = {
        ...rest,
        objectFit,
        zOrder: zOrder !== undefined ? zOrder : undefined,
        onDimensionsChange: onDimensionsChange as NativeProps['onDimensionsChange'],
    };

    if (iosPIP) {
        nativeProps.iosPIPEnabled = iosPIP.enabled ?? true;
        nativeProps.iosPIPStartAutomatically = iosPIP.startAutomatically ?? true;
        nativeProps.iosPIPStopAutomatically = iosPIP.stopAutomatically ?? true;

        if (iosPIP.preferredSize) {
            nativeProps.iosPIPPreferredWidth = iosPIP.preferredSize.width;
            nativeProps.iosPIPPreferredHeight = iosPIP.preferredSize.height;
        }
    }

    return <NativeRTCVideoView ref={ref} {...nativeProps} />;
});
